...
void loop() {
  ...
  if ( (recebido) && ((millis() - delayPrint) > 1000) ) {  //Mostra apenas após receber o primeiro sinal. Após o primeiro sinal, mostra a cada segundo.
     ...
     if (latitude != TinyGPS::GPS_INVALID_F_ANGLE) {
        ...
        Serial.println(latitude, 8);  //Mostra a latitude com a precisão de 6 dígitos decimais
     }

     if (longitude != TinyGPS::GPS_INVALID_F_ANGLE) {
        Serial.print("Longitude: ");
        Serial.println(longitude, 8);  //Mostra a longitude com a precisão de 6 dígitos decimais
     }
